...
try {
    ...
    $stmt = $db->prepare("SELECT s.*, t.table_code, u.id AS user_id, (CASE WHEN u.avatar_blob IS NOT NULL OR u.avatar IS NOT NULL THEN u.id ELSE NULL END) AS has_avatar FROM service_bookings s LEFT JOIN restaurant_tables t ON s.table_id = t.id LEFT JOIN users u ON s.user_id = u.id WHERE s.id = ?");
    $stmt->execute([$id]);
