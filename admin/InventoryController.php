<?php
// Tên file: admin/InventoryController.php (hoặc thư mục controllers tùy bạn sắp xếp)
...
if (isset($_GET['export_csv'])) {
    ...
    $rows = $db->query("SELECT i.*, s.name as s_name FROM inventory i LEFT JOIN suppliers s ON i.supplier_id = s.id ORDER BY i.item_name")->fetchAll(PDO::FETCH_ASSOC);
    foreach ($rows as $r) {
        fputcsv($out, [$r['item_name'], $r['category'], $r['unit_name'], $r['stock_quantity'], $r['min_stock'] ?? 0, $r['cost_price'], $r['s_name'] ?? 'Chưa gán', $r['expiry_date'] ?? '']);
    }
    fclose($out); exit;
}
...
if (isset($_POST['manage_tag'])) {
    $allowed_tables = ['category' => 'inventory_categories', 'unit' => 'inventory_units'];
    ...
    if ($table) {
        if ($_POST['tag_action'] == 'add') {
            ...
            $db->prepare("UPDATE $table SET name = ? WHERE id = ?")->execute([trim($_POST['tag_name']), (int)$_POST['tag_id']]);
        } elseif ($_POST['tag_action'] == 'delete') {
            $db->prepare("DELETE FROM $table WHERE id = ?")->execute([(int)$_POST['tag_id']]);
        }
    }
...
if (isset($_POST['save_inventory'])) {
    ...
    $data = [trim($_POST['item_name']), $_POST['category'], $_POST['unit_name'], (float)$_POST['cost_price'], $supplier_id, $min_stock];
    if (!empty($_POST['item_id'])) {
        ...
        $db->prepare("INSERT INTO inventory (item_name, category, unit_name, cost_price, supplier_id, min_stock, stock_quantity) VALUES (?, ?, ?, ?, ?, ?, 0)")->execute($data);
    }
    header("Location: InventoryController.php"); exit;
}

// F. Nhập / Xuất / Hủy (AJAX)
if (isset($_POST['action'])) {
    ...
    try {
        ...
        if ($_POST['action'] == 'import') {
            ...
            $avg_price = (($old_data['stock_quantity'] + $qty) > 0) ? ((($old_data['stock_quantity'] * $old_data['cost_price']) + ($qty * $price)) / ($old_data['stock_quantity'] + $qty)) : $price;

            $db->prepare("UPDATE inventory SET stock_quantity = stock_quantity + ?, cost_price = ?, supplier_id = ?, expiry_date = ? WHERE id = ?")->execute([$qty, $avg_price, $s_id, $_POST['expiry_date'] ?: null, $id]);
            $db->prepare("INSERT INTO inventory_history (ingredient_id, type, quantity, performed_by) VALUES (?, 'import', ?, ?)")->execute([$id, $qty, $current_user]);
        } else {
            $check = $db->prepare("SELECT stock_quantity FROM inventory WHERE id = ?");
            $check->execute([$id]);
            if ((float)$check->fetchColumn() < $qty) throw new Exception("Không đủ tồn kho!");
            
            $db->prepare("UPDATE inventory SET stock_quantity = stock_quantity - ? WHERE id = ?")->execute([$qty, $id]);
            $db->prepare("INSERT INTO inventory_history (ingredient_id, type, quantity, performed_by) VALUES (?, ?, ?, ?)")->execute([$id, $_POST['action'], $qty, $current_user]);
        }
        $db->commit(); echo json_encode(['status' => 'success']);
    } catch (Exception $e) { $db->rollBack(); echo json_encode(['status' => 'error', 'msg' => $e->getMessage()]); }
    exit;
}
...
if (isset($_POST['perform_audit'])) {
    $db->beginTransaction();
    try {
        $insAudit = $db->prepare("INSERT INTO inventory_audits (performed_by, notes) VALUES (?, ?)");
        $insAudit->execute([$current_user, $_POST['audit_notes']]);
        $audit_id = $db->lastInsertId();

        foreach ($_POST['actual_qty'] as $ing_id => $physical_qty) {
            if ($physical_qty === '') continue;
            $physical_qty = (float)$physical_qty;
            
            $st = $db->prepare("SELECT stock_quantity FROM inventory WHERE id = ?");
            $st->execute([$ing_id]);
            $system_qty = (float)$st->fetchColumn();
            $variance = $physical_qty - $system_qty;

            $db->prepare("INSERT INTO inventory_audit_details (audit_id, ingredient_id, system_qty, physical_qty, variance) VALUES (?, ?, ?, ?, ?)")->execute([$audit_id, $ing_id, $system_qty, $physical_qty, $variance]);

            if ($variance != 0) {
                $db->prepare("UPDATE inventory SET stock_quantity = ? WHERE id = ?")->execute([$physical_qty, $ing_id]);
                $db->prepare("INSERT INTO inventory_history (ingredient_id, type, quantity, performed_by) VALUES (?, ?, ?, ?)")->execute([$ing_id, ($variance > 0) ? 'import' : 'loss', abs($variance), $current_user . " (Kiểm kê)"]);
            }
        }
        $db->commit(); header("Location: InventoryController.php?tab=audit&msg=success");
    } catch (Exception $e) { $db->rollBack(); die("Lỗi kiểm kê: " . $e->getMessage()); }
    exit;
}

// --- 2. TRUY VẤN DỮ LIỆU ĐỂ HIỂN THỊ TRÊN VIEW ---
...
$history   = $db->query("SELECT h.*, i.item_name, i.unit_name FROM inventory_history h JOIN inventory i ON h.ingredient_id = i.id ORDER BY h.created_at DESC LIMIT 100")->fetchAll(PDO::FETCH_ASSOC);

$reorder_list = $db->query("SELECT i.*, (i.min_stock - i.stock_quantity) as suggest_qty, s.name as s_name FROM inventory i LEFT JOIN suppliers s ON i.supplier_id = s.id WHERE i.min_stock > 0 AND i.stock_quantity <= i.min_stock ORDER BY (i.min_stock - i.stock_quantity) DESC")->fetchAll(PDO::FETCH_ASSOC);

$today     = date('Y-m-d');
$warn_date = date('Y-m-d', strtotime('+7 days'));
$low_stock_count = 0; $expiry_warn_count = 0;
foreach ($inv as $i) {
    if ((float)($i['min_stock'] ?? 0) > 0 && (float)$i['stock_quantity'] <= (float)$i['min_stock']) $low_stock_count++;
...
// --- 3. NẠP VIEW ĐỂ HIỂN THỊ GIAO DIỆN ---
// Trỏ đường dẫn tới file View HTML của bạn
