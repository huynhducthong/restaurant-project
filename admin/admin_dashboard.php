...
if ($filter_date && preg_match('/^\d{4}-\d{2}-\d{2}$/', $filter_date)) {
    $conditions[] = "DATE(created_at) = ?"; $params[] = $filter_date;
}
if ($filter_month >= 1 && $filter_month <= 12) {
    $conditions[] = "MONTH(created_at) = ?"; $params[] = $filter_month;
}
if ($filter_quarter >= 1 && $filter_quarter <= 4) {
    $conditions[] = "QUARTER(created_at) = ?"; $params[] = $filter_quarter;
}
