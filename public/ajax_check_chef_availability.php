...
if ($check_chef_stmt->rowCount() > 0) {
    echo json_encode(["status" => "success", "available" => false, "message" => "Bếp trưởng đã có lịch trình vào khung giờ này. Vui lòng chọn thời gian khác cách ít nhất 8 tiếng, hoặc chọn Đầu bếp khác!"]);
} else {
    echo json_encode(["status" => "success", "available" => true]);
}
