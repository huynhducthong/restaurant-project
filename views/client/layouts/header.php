...
if (isset($_SESSION['user_id'])) {
    ...
    $stmt_quoted_g = $db->prepare("SELECT id FROM service_bookings WHERE user_id = ? AND status = 'Pending' AND ai_suggested_menu IS NOT NULL AND ai_suggested_menu != '' AND combo_id = -1 AND is_waiting_customer = 1 AND (chef_requirements IS NULL OR chef_requirements NOT LIKE '%[Khách hàng ĐÃ ĐỒNG Ý thực đơn]%') ORDER BY id DESC LIMIT 1");
